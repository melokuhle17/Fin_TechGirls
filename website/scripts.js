...
document.getElementById('calculate').addEventListener('click', () => {
  ...
  const selectedItem = data.items.find(item => item.description === selectedDescription);

      // Update the 'exchangeRate' and 'fee' spans with exchange rate and fee information
      const exchangeRateSpan = document.getElementById('exchangeRate');
      const feeSpan = document.getElementById('fee');
      if (selectedItem) {
        exchangeRateSpan.textContent = `Exchange Rate: ${selectedItem.rate.rate}`;
        feeSpan.textContent = `Fee: ${selectedItem.fee.amount} ${selectedItem.fee.currencyCode}`;
      } else {
        exchangeRateSpan.textContent = 'Exchange Rate: N/A';
        feeSpan.textContent = 'Fee: N/A';
      }
    });
// Calculate function to be triggered when the Calculate button is clicked
document.getElementById('calculate').addEventListener('click', () => {
  ...
  filteredItems.forEach(item => {
    const exchangeRate = item.rate.rate;
    const fee = item.fee.amount;

    // Calculate subtotal using the formula (sendAmount - fee) * exchangeRate
    const subtotal = (sendAmount - fee) * exchangeRate;
    ...
    // paragraph.textContent += `Exchange Rate: ${exchangeRate}\n`;
    // paragraph.textContent += `Fee: ${fee} ${item.fee.currencyCode}\n`;
    ...
    paragraph.textContent += `Subtotal: ${subtotal.toFixed(2)} ${item.payOutCurrencyCode}\n`;
